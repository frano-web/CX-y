...
function entryFeeBadge(r){ const paid=Boolean(r.entry_fee_paid); return `<span class="badge ${paid?'ok':'danger'}">🎟 ${paid?'startowe opłacone':'startowe nieopłacone'}</span>`; }
...
async function init(){
  ...
  if('serviceWorker' in navigator){
    navigator.serviceWorker.register('./sw.js?v=15',{updateViaCache:'none'}).catch(e=>console.warn('SW register',e));
  }
...
function activityMessage(a){
  ...
  if(a.entity_type==='races'){
    ...
    if(activityChanged(a,'entry_fee_paid'))return `<b>${who}</b> zmienił(a) startowe przy <strong>${label}</strong> na <strong>${n.entry_fee_paid?'opłacone':'nieopłacone'}</strong>`; if(activityChanged(a,'status'))return `<b>${who}</b> zmienił(a) status wyścigu <strong>${label}</strong>`;
...
function bindGlobal(){
  document.querySelectorAll('[data-nav]').forEach(b=>b.addEventListener('click',()=>{state.view=b.dataset.nav;state.selectedRaceId=null;if(['dashboard','calendar','races','tasks','results','costs','activity','notes','team'].includes(state.view))history.replaceState(null,'','#'+state.view);render();if(state.view==='activity'){refreshPushState(false).then(()=>render()).catch(e=>console.warn('Push activity',e));}}));
...
function bindRaceDetail(){
  ...
  document.querySelectorAll('[data-toggle-entry-fee]').forEach(x=>x.addEventListener('click',async()=>{const race=state.races.find(r=>r.id===x.dataset.toggleEntryFee);if(!race)return;const next=!Boolean(race.entry_fee_paid);await updateRow('races',race.id,{entry_fee_paid:next},'races');toast(next?'Startowe oznaczone jako opłacone':'Startowe oznaczone jako nieopłacone');}));
...
function openRaceModal(r=null){
  ...
  el.querySelector('#raceForm').addEventListener('submit',async e=>{e.preventDefault();const f=Object.fromEntries(new FormData(e.currentTarget));const row={team_id:state.team.id,name:f.name,race_date:f.race_date,start_time:f.start_time||null,city:f.city||null,category:f.category||null,race_address:f.race_address||null,registration_url:f.registration_url||null,entry_fee_paid:f.entry_fee_paid==='true',status:f.status,hotel_status:f.hotel_status,hotel_stay_days:Number(f.hotel_stay_days||1),hotel_arrival_days_before:Number(f.hotel_arrival_days_before??1),hotel_payment_status:f.hotel_payment_status||'nierozliczone',hotel_name:f.hotel_name||null,hotel_address:f.hotel_address||null,hotel_rooms:f.hotel_rooms||null,hotel_price:f.hotel_price?Number(f.hotel_price):null,hotel_url:f.hotel_url||null,hotel_notes:f.hotel_notes||null,race_notes:f.race_notes||null,transport_notes:f.transport_notes||null};try{if(editing)await saveRaceEdit(r.id,row);else await createRace(row);el.remove();render();toast(editing?'Zapisano zmiany':'Dodano wyścig');}catch(x){toast(x.message,true)}});
